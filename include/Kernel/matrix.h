...
class SymMatrix3x3
{
 ...
 SymMatrix3x3(const coord& a, const coord& b, const coord& c);
...
class Matrix4x4
{
 ...
 void setRotation(const coord &, const coord&, const coord&, const coord&);

 //! Translation matrix from a vector
 void setTranslation(const coord &, const coord&, const coord&);

 Matrix4x4 operator*(const Matrix4x4&) const; //!< Returns the product of this and another matrix (rows by columns)

 void transform(coord *, coord *, coord *); //! Transform the vector by left-multiplication with the matrix
};
