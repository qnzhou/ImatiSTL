...
SymMatrix3x3::SymMatrix3x3(const coord& x, const coord& y, const coord& z)
{
	M[0] = TMESH_TO_DOUBLE(x*x);  M[1] = TMESH_TO_DOUBLE(x*y);  M[3] = TMESH_TO_DOUBLE(x*z);
	M[2] = TMESH_TO_DOUBLE(y*y);  M[4] = TMESH_TO_DOUBLE(y*z);
    M[5] = TMESH_TO_DOUBLE(z*z);
}
...
Matrix4x4::Matrix4x4(const coord& d)
{
...
Matrix4x4::Matrix4x4(const coord& a11, const coord& a12, const coord& a13, const coord& a14,
		     const coord& a21, const coord& a22, const coord& a23, const coord& a24, 
		     const coord& a31, const coord& a32, const coord& a33, const coord& a34, 
		     const coord& a41, const coord& a42, const coord& a43, const coord& a44)
{
...
void Matrix4x4::setRotation(const coord& rx, const coord& ry, const coord& rz, const coord& rw)
{
  matrix[0][0] = rw*rw + rx*rx - ry*ry - rz*rz;
  matrix[0][1] =  rx*ry * 2 +  rw*rz * 2;
  matrix[0][2] =  rx*rz * 2 -  rw*ry * 2;
  matrix[0][3] = 0.0;

  matrix[1][0] =  rx*ry * 2 -  rw*rz * 2;
  matrix[1][1] = rw*rw - rx*rx + ry*ry - rz*rz;
  matrix[1][2] =  ry*rz * 2 +  rw*rx * 2;
  matrix[1][3] = 0.0;

  matrix[2][0] =  rx*rz * 2 +  rw*ry * 2;
  matrix[2][1] =  ry*rz * 2 -  rw*rx * 2;
...
void Matrix4x4::setTranslation(const coord& x, const coord& y, const coord& z)
{
...
void Matrix4x4::transform(coord *x, coord *y, coord *z)
{
 coord w, a = *x, b = *y, c = *z;
